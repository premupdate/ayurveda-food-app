...
Classify Ainthinai: Kurinji(mountains/hills), Mullai(forest/pastoral), Marutham(agricultural plains), Neidhal(coastal), Palai(arid/desert).
Return JSON: {{"area_name":"{api_data['area']}","area_name_tamil":"Tamil name","specific_land":"Kurinji/Mullai/Marutham/Neidhal/Palai","elevation":"Plains/Foothills/Hills/Coastal/Dry Plains","weather_location":"{api_data['district']}","notes":"Brief terrain"}}""")
    if land_result:
        try:
            c = db(); cur = c.cursor()
            cur.execute("INSERT INTO pincode_land_mapping (pincode,area_name,area_name_tamil,district_name,specific_land,elevation_category,weather_location,notes) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
                (pincode,land_result.get('area_name',''),land_result.get('area_name_tamil',''),api_data['district'],land_result.get('specific_land','Marutham'),land_result.get('elevation',''),land_result.get('weather_location',api_data['district']),land_result.get('notes','')))
...
# ==================== SIDEBAR: PINCODE-ONLY ====================
st.sidebar.title("🌿 Tamil Ayurvedic Platform")
st.sidebar.markdown("🆓 100% FREE | AI + Siddha + Weather")
st.sidebar.markdown("---")

typed_pin = st.sidebar.text_input("📮 Enter Your Pincode", "", placeholder="e.g., 625531")

# Auto-detect everything from pincode
specific_land = "Marutham"
weather_loc = "Chennai"
pin_area = ""
pin_district = ""
pin_elevation = ""
...
if typed_pin.strip() and len(typed_pin.strip()) == 6:
    ...
    db_pin = get_pincode_land(pincode)
    
    if db_pin:
        pin_area = db_pin[1]; pin_tamil = db_pin[2]; specific_land = db_pin[3]
        pin_elevation = db_pin[4]; weather_loc = db_pin[5]; pin_notes = db_pin[6]
        # Get district from API
        api_data = lookup_pincode(pincode)
        pin_district = api_data['district'] if api_data else ""
        
        st.sidebar.success(f"📍 **{pin_area}**")
        if pin_tamil: st.sidebar.markdown(f"   {pin_tamil}")
        if pin_district: st.sidebar.markdown(f"📍 District: **{pin_district}**")
        st.sidebar.markdown(f"{land_icons.get(specific_land,'🌍')} Land: **{specific_land}** ({pin_elevation})")
        if pin_notes: st.sidebar.markdown(f"📝 {pin_notes}")
    else:
        # Call India Post API
        with st.sidebar.status("🔍 Looking up pincode..."):
            api_data = lookup_pincode(pincode)
            if api_data:
                pin_area = api_data['area']
                pin_district = api_data['district']
                st.sidebar.info(f"📮 Found: **{pin_area}**, {pin_district}")
                
                # Classify land with AI
                land_result = classify_and_save_pincode(pincode, api_data)
                if land_result:
                    specific_land = land_result.get('specific_land','Marutham')
                    weather_loc = land_result.get('weather_location', pin_district)
                    pin_elevation = land_result.get('elevation','')
                    pin_tamil = land_result.get('area_name_tamil','')
                    
                    st.sidebar.success(f"📍 **{pin_area}** ({pin_tamil})")
                    st.sidebar.markdown(f"📍 District: **{pin_district}**")
                    st.sidebar.markdown(f"{land_icons.get(specific_land,'🌍')} Land: **{specific_land}** ({pin_elevation})")
                    st.sidebar.info("✅ Saved! Loads instantly next time.")
                else:
                    weather_loc = pin_district
                    st.sidebar.markdown(f"📍 **{pin_area}**, {pin_district}")
            else:
                st.sidebar.warning("❌ Pincode not found. Check and retry.")
...
# REORDERED NAVIGATION - Food Log FIRST
page = st.sidebar.radio("Navigate",["📝 Daily Food Log","👨‍🍳 Get Recommendations","👶 Kids Nutrition","🏔️ Ainthinai","🌦️ Seasons","🌿 Herbs","🔧 Admin"])

# ==================== PAGE 1: DAILY FOOD LOG (TRAINING) ====================
if page=="📝 Daily Food Log":
    st.title("📝 Daily Food Log - Help AI Learn!")
    st.markdown("Log what you eat every day. **AI learns from your food choices** to give better recommendations.")
    st.markdown("*The more you log, the smarter AI becomes!*")

    if not typed_pin.strip():
        st.warning("👆 Enter your pincode in the sidebar first!")

    # Show auto-detected context
    if pin_area or pin_district:
        col1,col2,col3,col4 = st.columns(4)
        col1.metric("📍 Area", pin_area if pin_area else "Enter pincode")
        col2.metric("📍 District", pin_district if pin_district else "-")
        col3.metric(f"{land_icons.get(specific_land,'🌍')} Land", specific_land)
        col4.metric("🌦️ Season", ts[1].split("(")[0].strip())

    st.markdown("---")

    with st.form("food_log"):
        st.subheader("☀️ Morning (Breakfast)")
        morning_notes = st.text_area("What you ate & WHY:","",height=80,key="mn",
            placeholder="Example: Had thulasi kadaisal for running nose. Kanji with inji for cold.")
        col1,col2 = st.columns(2)
        with col1: morning_rating = st.slider("How was it?",1,5,3,key="mr")
        with col2: morning_helped = st.selectbox("Did it help your health?",["Yes","Partially","No","Not for health"],key="mh")

        st.markdown("---")
        st.subheader("🌞 Afternoon (Lunch)")
        afternoon_notes = st.text_area("What you ate & WHY:","",height=80,key="an",
            placeholder="Example: Manathakkali sambar with rice for stomach pain. Carrot poriyal for vitamins.")
        col1,col2 = st.columns(2)
        with col1: afternoon_rating = st.slider("How was it?",1,5,3,key="ar")
        with col2: afternoon_helped = st.selectbox("Did it help?",["Yes","Partially","No","Not for health"],key="ah")

        st.markdown("---")
        st.subheader("🌙 Evening (Dinner)")
        evening_notes = st.text_area("What you ate & WHY:","",height=80,key="en",
            placeholder="Example: Ragi porridge with honey and ghee for weakness and body pain.")
        col1,col2 = st.columns(2)
        with col1: evening_rating = st.slider("How was it?",1,5,3,key="er")
        with col2: evening_helped = st.selectbox("Did it help?",["Yes","Partially","No","Not for health"],key="eh")

        st.markdown("---")
        st.subheader("🍟 Snacks & Junk Food")
        st.markdown("⚠️ **Be honest!** AI shows health impact & suggests healthy Tamil alternatives")
        junk_notes = st.text_area("What junk/snacks did you or family have?","",height=80,key="jn",
            placeholder="Example: Chips at 11am, pepsi with lunch, kids had kurkure and frooti at 4pm")
        ...
        with col3: fb_sleep = st.selectbox("Sleep Last Night",["Good","Normal","Disturbed","Poor"],key="fsl")
        ...
        submitted = st.form_submit_button("🤖 Submit & Let AI Learn",type="primary")
        ...
        if submitted:
            ...
            meal_results = {}
            if has_meals:
                with st.spinner("🤖 AI parsing your meals..."):
                    for slot,notes_text in [("morning",morning_notes),("afternoon",afternoon_notes),("evening",evening_notes)]:
                        if notes_text.strip():
                            parsed = ai(f"""Tamil food analyst. Parse: "{notes_text}". Time: {slot}. Location: {pin_area} {pin_district} ({specific_land}). Season: {fb_ts[1]}.
Return JSON: {{"dishes":[{{"name":"Dish","dish_type":"Type","vegetables":["v"],"leaves_greens":["l"],"herbs":["h"],"spices":["s"],"grains":["g"],"symptom_treated":"Symptom or empty","symptom_dosha":"Dosha or empty"}}],"overall_dosha_impact":"Impact","ai_note":"Siddha insight for this land and season"}}""")
                            meal_results[slot] = parsed

            # Parse junk
            junk_result = None; junk_count = 0
            if has_junk:
                with st.spinner("🤖 Analyzing junk food impact..."):
                    junk_result = ai(f"""Nutritionist. Junk consumed: "{junk_notes}". Healthy meals today: {all_notes}. Location: {pin_area} ({specific_land}).
Return JSON: {{"items":[{{"name":"Item","calories":"est","sugar_g":"est","sodium_mg":"est","harmful_chemicals":"list","dosha_impact":"impact","healing_interference":"how it reduces healthy food benefit","consumed_by":"Adult/Kids/Both","tamil_alternative":"healthy Tamil replacement","alternative_benefit":"why better"}}],"total_junk_count":3,"daily_health_reduction_percent":35,"kids_impact":"impact on children if applicable","overall_message":"summary advice"}}""")
            ...
            health_score = min(10,max(1,(meal_count*2)+(helped_count*2)-(junk_count)+(fb_energy//3)))

            # Save to database
            try:
                conn=db();cur=conn.cursor()
                cur.execute("""INSERT INTO feedback_detailed (user_id,feedback_date,pincode,area_name,specific_land,tamil_season,weather_condition,
                    morning_notes,morning_rating,morning_helped,morning_parsed,
                    afternoon_notes,afternoon_rating,afternoon_helped,afternoon_parsed,
                    evening_notes,evening_rating,evening_helped,evening_parsed,
                    junk_notes,junk_parsed,junk_count,energy_level,digestion,sleep_quality,mood,daily_health_score)
                    VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
                    (fb_user_id,date.today(),typed_pin.strip(),f"{pin_area}, {pin_district}",specific_land,fb_ts[1],f"{fb_w['temp']}C {fb_w['desc']}",
                     morning_notes,morning_rating,morning_helped,json.dumps(meal_results.get('morning')) if meal_results.get('morning') else None,
                     afternoon_notes,afternoon_rating,afternoon_helped,json.dumps(meal_results.get('afternoon')) if meal_results.get('afternoon') else None,
                     evening_notes,evening_rating,evening_helped,json.dumps(meal_results.get('evening')) if meal_results.get('evening') else None,
                     junk_notes,json.dumps(junk_result) if junk_result else None,junk_count,fb_energy,fb_digestion,fb_sleep,fb_mood,health_score))
            ...
            for slot,icon,notes_text,rating,helped in [("morning","☀️",morning_notes,morning_rating,morning_helped),("afternoon","🌞",afternoon_notes,afternoon_rating,afternoon_helped),("evening","🌙",evening_notes,evening_rating,evening_helped)]:
                if slot in meal_results and meal_results[slot]:
                    st.markdown("---"); st.subheader(f"{icon} {slot.title()} Analysis")
            ...
            if junk_result and junk_result.get('items'):
                ...
                for item in junk_result['items']:
                    ...
                    if 'kid' in item.get('consumed_by','').lower(): st.error("👶 Extra harmful for children!")
                    st.success(f"🌿 **Tamil Alternative:** {item.get('tamil_alternative','')} → {item.get('alternative_benefit','')}")
            ...
            if health_score>=8: st.success("🌟 Excellent! Traditional meals are working!")
            elif health_score>=5: st.info("👍 Good effort! Less junk = better score.")
            else: st.warning("⚠️ Too much junk today. Try Tamil snacks tomorrow.")
            st.success("✅ AI is learning from your food log! Better recommendations coming soon.")

# ==================== PAGE 2: GET RECOMMENDATIONS ====================
elif page=="👨‍🍳 Get Recommendations":
    st.title("🌿 Get Personalized Food Recommendations")
    if not typed_pin.strip():
        st.warning("👆 Enter your pincode in the sidebar first!")

    col1,col2,col3,col4 = st.columns(4)
    col1.metric("📍",pin_area if pin_area else "Enter pincode")
    ...
    if local_flora:
        st.info(f"🌿 **Available in {specific_land} now:** {', '.join([f[2] for f in local_flora[:8]])}")
    ...
    with st.form("body"):
        ...
        st.markdown("---"); st.header("📝 Health Conditions")
        ...
        health_notes = st.text_area("Health conditions","",height=80,placeholder="Example: Type 2 diabetes, knee arthritis, wife is pregnant")
        ...
        if st.form_submit_button("🤖 Get Recommendations",type="primary"):
            try:
                ...
                cur.execute("INSERT INTO body_condition_log (user_id,log_date,cold_intensity,cough_type,cough_severity,pain_locations,pain_severity,pimple_count,sweating_level,sputum_color,urine_color,energy_level,digestion_quality,weather_condition,health_notes,pincode,specific_land) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                    (user_id,date.today(),cold,cough,cough_sev,pain_locs,pain_sev,pimples,sweating,sputum,urine,energy,digestion,f"{weather_loc} {w['temp']}C {w['desc']}",health_notes,typed_pin.strip(),specific_land))
                conn.commit();cur.close();conn.close()
                st.success("✅ Logged!")

                parsed_conditions = []
                if health_notes.strip():
                    with st.spinner("🤖 Parsing health..."):
                        ...
                        if parsed and parsed.get('conditions'):
                            ...
                            for pc in parsed_conditions: st.write(f"⚠️ **{pc.get('name','')}** | ❌ {pc.get('avoid_herbs','')} | ✅ {pc.get('recommended_herbs','')}")
                ...
                with st.spinner("🤖 Classifying dosha..."):
                    dosha = ai(f"""Siddha expert. Body: {json.dumps(body)}, Location: {pin_area} ({specific_land}), Season: {ts[1]} ({ts[2]}), Weather: {w['temp']}C {w['desc']}, Health: {health_ctx}
Return JSON: {{"primary_dosha":"Vata/Pitta/Kapha","dosha_percent":75,"secondary_dosha":"X","secondary_percent":25,"confidence":0.9,"summary":"Brief","weather_impact":"X","season_impact":"X","land_impact":"X","health_impact":"X"}}""")
                    if dosha:
                        ...
                        if dosha.get('health_impact'): st.warning(f"⚠️ {dosha['health_impact']}")
                        ...
                        with st.spinner("🤖 Generating recipes..."):
                            recipes = ai(f"""Tamil chef. LOCAL recipes. Dosha: {dosha['primary_dosha']} ({dosha['dosha_percent']}%), Location: {pin_area} ({specific_land}), Season: {ts[1]}, Weather: {w['temp']}C, Energy: {energy}/10
...
# ==================== PAGE 3: KIDS ====================
    ...
    st.markdown(f"📍 {pin_area if pin_area else 'Enter pincode'} ({specific_land}) | 🌦️ {ts[1]} | 🌤️ {w['temp']}°C")
    col1,col2 = st.columns(2)
    ...
    if st.button("🤖 Get Local Meal Plan",type="primary"):
        ...
        with st.spinner("Creating..."):
            kids = ai(f"""Tamil pediatric nutritionist. Child {age}yr, Dosha {kid_dosha}. Location: {pin_area} ({specific_land}), Season: {ts[1]}
Local flora: {ft}. Kid herbs: {ht}
    ...
    try:
        ...
        for i,land in enumerate(lands):
            with tabs[i]:
                ...
                with col2:
                    st.write(f"👷 {land[12]}"); st.write(f"🍚 {land[14]}")
                    st.write(f"💕 {land[16]}"); st.write(f"📍 {land[18]}")
                ...
                for f in cur.fetchall():
                    ...
                    st.write(f"{ti} **{f[2]}** ({f[1]}) | {f[3]} | {f[4]} | {f[5]}")
...
# ==================== PAGE 7: ADMIN ====================
    ...
    try:
        ...
        col1.metric("🌿 Herbs",h);col2.metric("🍽️ Dishes",r);col3.metric("📋 Logs",l);col4.metric("📮 Pins",p);col5.metric("📝 Food Logs",f)
    except: pass
